...
Scene* MenuPick_1::createScene()
{
	return MenuPick_1::create();
}

bool MenuPick_1::init()
{
	if (!Scene::init())
	{
		return false;
	...
	//Backlayer,tag 0,菜单tag 1,难度选择tag 2,选择单选tag 3,分数tag 4,难度tag 5,等级tag 6,
	// 歌曲图像tag7,箭头tag8，速度tag9
	//箭头tag8
	...
	//曲名加到精灵上
	auto Pick1 = Label::createWithTTF(FontToUTF8("インフェルノシティ"), "fonts/simhei.ttf", 36);
	...
	auto Pick2 = Label::createWithTTF(FontToUTF8("千本桜"), "fonts/simhei.ttf", 36);
	...
	auto Pick3 = Label::createWithTTF(FontToUTF8("遊戯王5DS"), "fonts/simhei.ttf", 36);
	...
	auto MenuPick4 = MenuItemSprite::create(White4, White4,
		CC_CALLBACK_0(MenuPick_1::pickmusic, this, 4));
	auto Pick4 = Label::createWithTTF("Bella Ciao", "fonts/arial.ttf", 36);
	...
	auto MenuPick5 = MenuItemSprite::create(White5, White5,
		CC_CALLBACK_0(MenuPick_1::pickmusic, this, 5));
	auto Pick5 = Label::createWithTTF("La Marseillaise", "fonts/arial.ttf", 36);
	...
	auto MenuPick6 = MenuItemSprite::create(White6, White6,
		CC_CALLBACK_0(MenuPick_1::pickmusic, this, 6));
	auto Pick6 = Label::createWithTTF("La Campanella", "fonts/arial.ttf", 36);
	...
	auto MenuPick7 = MenuItemSprite::create(White7, White7,
		CC_CALLBACK_0(MenuPick_1::pickmusic, this, 7));
	...
	auto MenuPick8 = MenuItemSprite::create(White8, White8,
		CC_CALLBACK_0(MenuPick_1::pickmusic, this, 8));
	...
	auto Hd_Diff = MenuItemImage::create("Cover/Hd1.png", "Cover/Hd1.png",
		CC_CALLBACK_0(MenuPick_1::pickdiffprer, this, 2));
	auto In_Diff = MenuItemImage::create("Cover/In1.png", "Cover/In1.png",
		CC_CALLBACK_0(MenuPick_1::pickdiffprer, this, 3));
...
void MenuPick_1::pickmusic(int pick)
{
	...
	if (pick == 1)
	{
		music_name = "Inferno City -Ponch";
		pickarrow->setPosition(Vec2(515, 900));
	}
	else if (pick == 2)
	{
		music_name = "Senbonzakura -Lindsey Stirling";
		pickarrow->setPosition(Vec2(515, 800));
	}
	else if (pick == 3)
	{
		music_name = "Jack War -YGO5ds";
		pickarrow->setPosition(Vec2(515, 700));
	}
	else if (pick == 4)
	{
		music_name = "Bella Ciao -Kaiserreich";
		pickarrow->setPosition(Vec2(515, 600));
	}
	else if (pick == 5)
	{
		music_name = "La Marseillaise -French national anthem";
		pickarrow->setPosition(Vec2(515, 500));
	}
	else if (pick == 6)
	{
		music_name = "La Campanella - SunsetRay";
...
void MenuPick_1::pickspeed(int pick)
{
...
void MenuPick_1::pickdiffprer(int pick)
{
	Pick_diff = pick;
...
void MenuPick_1::backmeun()
{
	Director::getInstance()->popScene();
}

char* MenuPick_1::FontToUTF8(const char* font)
{
...
void MenuPick_1::gameplay(Ref* pSender)
{
	Filename = music_name;
	if (Pick_diff == 1)
	{
...
void MenuPick_1::restartplay(Ref* pSender)
{
	PLay_Back = 0;
	Filename = music_name;
	if (Pick_diff == 1)
